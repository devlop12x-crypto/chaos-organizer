...
<!-- TODO: добавить скриншот docs/screenshots/transcript.png — живые субтитры
     снимаются только в настольном Chrome (нужен сервис распознавания Google),
     в автоматизации этот кадр не получить. -->
...
Кнопка-замок в панели ввода включает режим шифрования: приложение один раз запрашивает пароль и с этого момента **шифрует на клиенте** каждое отправленное сообщение и каждый файл — сервер получает и хранит только непрозрачный шифротекст, пароль и ключ на сервер никогда не уходят.

Схема реализована на **нативном Web Crypto API** (`crypto.subtle`) — без единой сторонней библиотеки:

- **Деривация ключа:** PBKDF2-HMAC-SHA256, **600 000 итераций** (рекомендация OWASP), случайная 16-байтовая соль. Подбор пароля упирается в стоимость итераций, соль исключает rainbow-таблицы.
- **Шифр:** **AES-256-GCM** — это AEAD: помимо конфиденциальности даёт аутентификацию. Подмена хотя бы одного байта шифротекста или неверный пароль гарантированно выявляются при расшифровке (исключение на проверке тэга), а не превращаются в «мусорный» текст.
- **Случайность:** свежие соль и 12-байтовый IV на каждое сообщение и файл — повтор пары (ключ, IV), недопустимый для GCM, исключён криптостойким генератором.
- **Форматы:** сообщение — `base64(соль ‖ IV ‖ шифротекст с тэгом)`; файл — самодостаточный бинарный контейнер `[ 'CHAO' | версия | соль | IV | шифротекст ]`, ~25% компактнее текстового base64.
- **Производительность:** деривация (~секунда) выполняется один раз на пароль — ключ кэшируется в памяти вкладки, отправка остаётся мгновенной. На стойкости это не сказывается: соль хранится в каждом шифротексте (расшифровка самодостаточна), IV всегда свежий.
...
**Границы модели** (проговорено честно): это **не end-to-end шифрование** в строгом смысле (как в секретных чатах Telegram/Signal) — там ключ согласуется между устройствами через Диффи-Хеллмана, а здесь общий пароль нужно передавать собеседнику отдельно, вне приложения. Метаданные (имена файлов, MIME, размер) остаются открытыми — шифруется только содержимое. Web Crypto API требует secure context (HTTPS или localhost) — для демо на GitHub Pages и сервера на Render это выполняется всегда.
...
### 14. Тёмная тема (сверх ТЗ)

Кнопка с луной/солнцем в шапке переключает светлую и тёмную темы. Вся палитра приложения собрана на CSS-переменных, поэтому тема меняется мгновенно — переопределением набора переменных в `[data-theme="dark"]`, без дублирования стилей. Выбор запоминается в `localStorage`; при первом визите тема следует системной настройке (`prefers-color-scheme`), а применяется ещё до первой отрисовки страницы — без «вспышки» неверной темы при загрузке.

![Тёмная тема](docs/screenshots/theme-dark.png)
...
**Криптография:** нативный Web Crypto API (`crypto.subtle`), без сторонних библиотек — AES-256-GCM (AEAD) + PBKDF2-HMAC-SHA256 (600 000 итераций, случайные соль и IV). Подробная схема — в пункте 10 выше.

**Сервер:** Node.js 18+, Koa (@koa/router, @koa/cors, koa-body), ws. Данные — **SQLite** (better-sqlite3), файлы — на диске.
...
└── js/
    ...
    │   └── theme.js                 # светлая/тёмная тема (localStorage, prefers-color-scheme)
...
Одна команда поднимает сразу фронтенд и бэкенд (ожидается, что оба репозитория склонированы рядом в одну папку):

```bash
docker compose up --build
```

Приложение откроется на http://localhost:8080 — фронтенд сам подключится к API на порту 7070 (см. `src/js/config.js`). Фронтенд собирается webpack'ом и отдаётся nginx'ом (multi-stage build, без node и исходников в рантайм-образе), бэкенд хранит базу SQLite и загруженные файлы на именованных volume'ах — данные переживают пересоздание контейнеров.

## Известные особенности

- Данные хранятся в **SQLite** на сервере (better-sqlite3, файл `data/chaos.db`, путь настраивается через `DB_PATH`) и переживают перезапуск/редеплой; при пустой базе сервер наполняет её демоданными. Файлы вложений — на диске в `uploads/`.
